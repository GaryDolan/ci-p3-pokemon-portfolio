...
Pokemon Portfolio is a web page terminal application that allows users to collect and appraise their base set pokemon cards.
...
-   To create a high level of user feedback while using the app, ensuring the user knows what they can do and what they have done when using the app.
...
Due to the fact that this application was terminal based, colour choices were limited. The primary objective I wanted to achieve with my colour scheme was to use it as a form of user feedback. I would use green to indicate a positive action or message and red to indicate a negative action or message.

A secondary goal of my colour scheme was to add some style to the application. As it was based mostly in black and white I wanted to add some colour to the app. I used yellow for the page titles and pokemon names when searched. I choose yellow because it is strongly associated with the pokemon brand and had the best visibility when using ascii art fonts.

I used [Coolors](https://coolors.co/) to visualise my palette.
...
Due to the fact that images cannot be displayed in the terminal, I wanted to ensure I used strong bold colours in my webpage [background](https://www.pixelstalk.net/pokemon-hd-backgrounds-free-download/). This adds contrast between the terminal and background and improved user experience.

I also wanted to add images of each of the 102 pokemon in the set throughout the app. To do this I use Emoji Combos [image to dot convertor](https://emojicombos.com/dot-art-generator). I generated card 1 to 69 using images from [pokemon.com](https://www.pokemon.com/us/pokedex) and cards 70 to 102 using images from [tcgplayer.com](https://shop.tcgplayer.com/price-guide/pokemon/base-set-shadowless). Most of these images were edited manually after dot image generation. I discussed copy rights regarding the use of these images with both my mentor and cohort facilitator. Both agreed that there were no issues using the images, mainly because I never use the actual image and also because the app is only for educational purposes.
...
Due to the nature of the application the user's data needs to be retained. To do this I added a create an account feature which allows the users to create an account using a username, password and phone number (used for password recovery). Once the account is created a location in google sheet is assigned to store the information pertaining to that username.
...
This option allows a user to search for cards by their card number. Once a number is entered, an image of the card / pokemon added will be displayed. This will be accompanied by a table showing the card details, these are the number, name, rarity, price and if it is in the users collection.
![Image of ](/documentation/features)
...
-   [Credentials]() from google.oauth2.service_account module - Autenticate and authorise access to Google cloud services using a service account.
...
-   [bcrypt](https://pypi.org/project/bcrypt/) - Password creation using salt and password match checking.
-   [sys](https://docs.python.org/3/library/sys.html) - Imported for access to system-specific parameters and function such as exit().
-   [time](https://docs.python.org/3/library/time.html) - Access to sleep (delays) for terminal.
-   [colored]() from the termcolor module - Colour printing to terminal.
...
-   [EZGIF](https://ezgif.com/maker) was used to create gif's from recorded video for this readme.
...
This application was created using the [Code Institute Python Essentials Template](https://github.com/Code-Institute-Org/python-essentials-template). This template creates a webpage based template that can run python code. For the application to run using the template, the follow changes need to be made to the python code.
...
-   I want to test that the app is enjoyable and is fun to use.
    ...
    -   The app contains ascii font art, dot images and coloured text, along with custom background to encourage the user to have fun and enjoy their time using the app. Pokemon images chosen for feature pages are popular and well known so the user will recognise and enjoy viewing their images as they use the app.
...
-   I want the app to provide me with feedback regarding actions I have taken.
    -   Selections and inputs throught the app provide the user with a high level of feedback. Feedback is given when the user makes a correct selection or input and when the user makes and incorrect selection or input. Where relevant there is messaging when any action takes time, such as loading card detail..., logging in... etc.
-   I want the ability to log out when I am finished.
...
This bug was found during testing of add and remove cards featured. If 0 was entered before the given value it crashed the app. This was because the user input was taken and passed to the print pokemon function as a string. The funcntion then it looked to print a pokemon from the pokemon_ascii_art module using an invalid string.
...
During testing of the main menu I found a bug. When the user selected a invalid selection the error messages were not being displayed. The bug was caused by the way I structure the main menu function inside a large while true loop. This meant that after validation of the input the loop repeated and the first statement in the loop cleared the terminal. I fixed this by wrapping the printing, input and validating of the options in a while true loop. This meant that upon invalid selection the internal while loop would not break from the loop and the error messages were no longer cleared and could be viewed correctly.
